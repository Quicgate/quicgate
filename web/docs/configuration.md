...
| `QG_SECRET_KEY_FILE` | | file with the key that encrypts the secrets in the database (one base64 key of 32 bytes per line; the first seals, all open) |
| `QG_SECRET_KEY` | | the same key given directly as base64. Without either variable quicgate keeps `secret.key` in `QG_DATA` |
...
- WireGuard sites (upstreams reached through a WireGuard peer on another network) are set up on the VPN page, not by environment variables: see the Streams & port forwards guide. The settings are `wg_enabled`, `wg_port`, `wg_network` and `wg_endpoint`.
- Devices, the VPN portal and LAN access are described in the [VPN guide](vpn.md). Their settings: `wg_lease_minutes` (10), `wg_outage_grace_minutes` (0), `wg_session_days` (30), `wg_auth_max_age` (900 seconds), `wg_devices_per_user` (5), `wg_lan_access` (off), `wg_protected_endpoints` and `wg_no_published_aliases`. LAN flows are written to `logs/vpn-flows.log` in the data directory, and the addresses dial-in peers were last seen at to `wg-endpoints.json`.
- Secrets in the database (two-factor secrets, OIDC client secrets, DNS credentials, certificate private keys, the SSO cookie key) are encrypted with the key above. Backup archives never contain that key, so keep a copy of it with your backups, stored separately: a restore on another machine cannot use the sealed secrets without it. If the key is missing or wrong, quicgate keeps running with the secrets locked: nothing is replaced, and OIDC sign-in, DNS-01, custom certificates and two-factor logins stop working until the key is back. To go back to a version from before 1.15.0, run `quicgate -unseal` first. The ACME certificate files under `certs/` are not encrypted; treat the data directory and its backups as sensitive.
