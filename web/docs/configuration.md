...
- Secrets in the database (two-factor secrets, OIDC client secrets, DNS credentials, certificate private keys, the SSO cookie key) are encrypted with the key above. Backup archives never contain that key, so keep a copy of it with your backups, stored separately: a restore on another machine cannot use the sealed secrets without it. If the key is missing or wrong, quicgate keeps running with the secrets locked: nothing is replaced, and OIDC sign-in, DNS-01, custom certificates and two-factor logins stop working until the key is back. To go back to a version from before 1.15, run `quicgate -unseal` first. The ACME certificate files under `certs/` are not encrypted; treat the data directory and its backups as sensitive.
