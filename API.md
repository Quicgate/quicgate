...
Keys: `acme_email`, `acme_staging` (`"1"`/`"0"`), `acme_ca_url`, `acme_dns_provider`, `acme_dns_config`, `notify_url`, `default_site` (`404|html|redirect`), `default_site_value`, `ban_enabled`, `ban_threshold`, `ban_window_sec`, `ban_duration_sec`, `ban_exempt` (addresses and CIDR ranges that are never banned, separated by newlines, commas or spaces; `400` when an entry is neither), `ban_exempt_own` (`"1"` = never ban this machine's own addresses), `oidc_*`, `ldap_*`.
...
| GET | `/api/bans` | Addresses auto-ban is refusing now, most recent first: `[{ip, country?, since, until, failures, host, reason}]`. `reason` says what the last refused request failed (`address not allowed by access list "home"`, `no credentials for ...`, `wrong credentials for ...`), `host` is the host it asked for, `country` appears with a GeoIP database. Empty while auto-ban is off. Bans are saved to `bans.json` in the data directory when they start or are lifted, and restored at startup without the ones that expired. |
| GET | `/api/own-addresses` | The addresses `ban_exempt_own` covers now: `[{ip, source}]`, `source` is `router` for the public address the router reports over UPnP, `interface` for an address of this machine. |
| DELETE | `/api/bans/{ip}` | Lifts the ban on that address and forgets its recent refusals; `400` for something that is not an IP address, `404` when it is not banned. |
| GET | `/api/traffic?range=1h` | Traffic history behind the Overview charts; `range` is `1h` (10-second points), `6h` or `24h` (5-minute points) or `7d` (hourly points), any other value `400`. Returns `{range, step, start, lastSpan, series, totals, ports[], hosts[], countries[], geoip, open, banned}`. `series` holds one value per point, oldest first, with `null` where quicgate was not running: `secs` (seconds of traffic the point holds; divide by it for rates, since the newest point and one spanning a restart hold less than `step`), `in` and `out` (bytes on the client side of every listener), `requests`, `2xx` to `5xx`, `blocked`, `open` (most connections open at once), `p50` and `p95` (time to first byte in milliseconds). `lastSpan` is the newest point's `secs`. `totals` sums the range, with `protocols` and `blocked` by reason. `ports` describes every listener with its bytes, connections, open connections, state, whether UPnP mapped it (`exposed`, absent when UPnP is off) and a `spark` of bytes per second; `hosts` and `countries` are the ten busiest. |
...
| GET | `/metrics` | Prometheus exposition; requires admin authentication (a session, or `Authorization: Bearer <API token>` for scrapers). Counters: `quicgate_requests_total`, `quicgate_responses_total{class}`, `quicgate_response_bytes_total`, and per route `quicgate_host_requests_total{host}`, `quicgate_host_errors_total{host}`, `quicgate_host_response_bytes_total{host}`, where `host` is a configured domain, `*.suffix` for a wildcard route, or `_unmatched`. Per listener (`tcp:443`, `udp:443`, a stream's first port): `quicgate_listener_received_bytes_total`, `quicgate_listener_sent_bytes_total`, `quicgate_listener_connections_total` and the gauge `quicgate_listener_open_connections`; the HTTP/3 listener's are updated every 10 seconds. Also `quicgate_requests_by_protocol_total{protocol}` and `quicgate_blocked_total{reason}` (`accessList`, `banned`, `rateLimit`, `exploit`, `bot`, `clientCert`, `sso`, `forwardAuth`, `streamSource`). |
