...
| GET | `/api/bans` | Addresses auto-ban is refusing now, most recent first: `[{ip, country?, since, until, failures, host, reason}]`. `reason` says what the last refused request failed (`address not allowed by access list "home"`, `no credentials for ...`, `wrong credentials for ...`), `host` is the host it asked for, `country` appears with a GeoIP database. Empty while auto-ban is off. Bans are held in memory and end on restart. |
