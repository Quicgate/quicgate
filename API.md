...
| GET | `/api/traffic?range=1h` | Traffic history behind the Overview charts; `range` is `1h` (10-second points), `6h` or `24h` (5-minute points) or `7d` (hourly points), any other value `400`. Returns `{range, step, start, lastSpan, series, totals, ports[], hosts[], countries[], geoip, open, banned}`. `series` holds one value per point, oldest first, with `null` where quicgate was not running: `in` and `out` (bytes on the client side of every listener), `requests`, `2xx` to `5xx`, `blocked`, `open` (most connections open at once), `p50` and `p95` (time to first byte in milliseconds). The newest point may still be filling and covers `lastSpan` seconds. `totals` sums the range, with `protocols` and `blocked` by reason. `ports` describes every listener with its bytes, connections, open connections, state, whether UPnP mapped it (`exposed`, absent when UPnP is off) and a `spark` of bytes per second; `hosts` and `countries` are the ten busiest. |
